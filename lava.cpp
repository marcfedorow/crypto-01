...
uint32_t Lava::add_mod_32_minus_1(uint32_t a, uint32_t b){
	uint32_t res = a + b;
	return (a + b) + ((res < a) | (res < b));
}

uint32_t* Lava::string2key(char* str, int size){
...
uint64_t Lava::crypt(uint64_t a, uint32_t* key, bool encrypt, int rounds){
	data64 data;
	data._64 = a;
	uint32_t tmp;
	for (int i = 0; i < 4; ++i){
...
void Lava::simple(char* in, char* out, char* keystr, bool encrypt){
	auto key = string2key(keystr, 8);
	simple(in, out, key, encrypt);
}

void Lava::simple(char* in, char* out, data64* key64, bool encrypt){
	uint32_t key[8];
	for (int i = 0; i < 4; ++i){
...
void Lava::simple(char* in, char* out, uint32_t* key, bool encrypt){
...
void Lava::gamma(char* in, char* out, char* keystr,  char* init_vector, bool back, bool encrypt){
...
void Lava::gamma(char* in, char* out, data64* key64, data64 init64, bool back, bool encrypt){
...
void Lava::gamma(char* in, char* out, uint32_t* key, uint32_t* iv, bool back, bool encrypt){
	FILE* fin = fopen(in, "rb");
	if (!fin) {
		printf("Failed to open %s", in);
		return;
...
void Lava::test(){
	...
	*/
	...
	char* keystr = "ffeeddccbbaa99887766554433221100f0f1f2f3f4f5f6f7f8f9fafbfcfdfeff\0";
	uint32_t* key = string2key(keystr);
	for (int i = 0; i < 8 && 0; ++i){
		printf("%x\n", key[i]);
	}

	uint64_t cyphertext = crypt(0xfedcba9876543210ULL, key, 1);
	printf("c = %llx\n", cyphertext);
